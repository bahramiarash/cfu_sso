...
# Create shared HTTP session with connection pooling for better performance
...
def method_not_allowed(error):
    """Handle 405 Method Not Allowed errors (e.g., WebDAV PROPFIND requests)"""
    # Silently handle WebDAV methods (PROPFIND, PROPPATCH, etc.) to reduce log noise
    method = request.method if hasattr(request, 'method') else 'UNKNOWN'
    if method in ['PROPFIND', 'PROPPATCH', 'MKCOL', 'LOCK', 'UNLOCK', 'MOVE', 'COPY']:
        # These are WebDAV methods, silently return 405
        from flask import make_response
        response = make_response('', 405)
        response.headers['Allow'] = 'GET, POST, PUT, DELETE, PATCH, OPTIONS, HEAD'
        return response
    else:
        # Log other method errors but don't spam
        logger.debug(f"405 Method Not Allowed: {method} {request.path}")
        from flask import make_response
        response = make_response('', 405)
        response.headers['Allow'] = 'GET, POST, PUT, DELETE, PATCH, OPTIONS, HEAD'
        return response
...
def handle_exception(e):
    ...
    # Check if this is a MethodNotAllowed exception (405)
    from werkzeug.exceptions import MethodNotAllowed
    if isinstance(e, MethodNotAllowed):
        # This will be handled by the 405 error handler, so we can return early
        # But since we're already in the exception handler, we need to handle it here
        method = request.method if hasattr(request, 'method') else 'UNKNOWN'
        if method in ['PROPFIND', 'PROPPATCH', 'MKCOL', 'LOCK', 'UNLOCK', 'MOVE', 'COPY']:
            # Silently handle WebDAV methods
            from flask import make_response
            response = make_response('', 405)
            response.headers['Allow'] = 'GET, POST, PUT, DELETE, PATCH, OPTIONS, HEAD'
            return response
...
def static_files(filename):
    ...
    try:
        ...
        resp = get_shared_session().get(target_url, timeout=10, allow_redirects=False)
...
def knowledge_proxy(path):
    ...
    try:
        ...
        # Clear cookies from previous requests to avoid leakage
    ...
    try:
        ...
        if request.method == "GET":
            ...
            if request.is_json:
                ...
                # CRITICAL: Flask's request.form.getlist() should work, but let's also check raw data
                ...
                # Parse raw form data to get ALL values
                # Flask's request.form.getlist() should work, but let's verify
                form_data = []
                for key in request.form:
                    # Use getlist() to get ALL values for this key
                    values = request.form.getlist(key)
                    logger.info(f"Gateway POST: Key '{key}' - getlist() returned {len(values)} values: {values}")
                    if len(values) == 0:
                        # Fallback: try to parse from raw data
                        logger.warning(f"Gateway POST: getlist() returned empty for key '{key}', trying raw data parse")
                        # Parse manually from raw_data
                        from urllib.parse import parse_qs
                        parsed = parse_qs(raw_data)
                        if key in parsed:
                            values = parsed[key]
                            logger.info(f"Gateway POST: Parsed from raw data: {key} = {values}")
                    for value in values:
                ...
                if request.content_type and 'multipart/form-data' in request.content_type:
                    # Also add files if any
                    if request.files:
                        files_data = []
                        for key in request.files:
                            files_data.append((key, (request.files[key].filename, request.files[key].read(), request.files[key].content_type)))
...
def health():
    ...
    try:
        health_resp = get_shared_session().get(f"{MONOLITHIC_APP_URL}/health", timeout=2)
