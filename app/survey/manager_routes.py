...
from .utils import log_survey_action, sanitize_input, is_survey_manager, get_survey_manager
...
def manager_dashboard():
    ...
    try:
        manager = get_survey_manager(current_user)
        if not manager:
...
def manager_surveys_create():
    ...
    if request.method == 'POST':
        try:
            ...
            if request.form.get('start_date'):
                try:
                    ...
                    if jalali_str:
                        ...
                        jalali_dt = jdatetime(jalali_parts[0], jalali_parts[1], jalali_parts[2])
            ...
            # Handle logo upload
            # Handle logo upload - only process if a file with actual filename is uploaded
            logo_file = request.files.get('logo')
            # CRITICAL: Check if file exists AND has a non-empty filename
            # Flask creates file objects even when no file is uploaded, so we must check filename
            if logo_file:
                filename = getattr(logo_file, 'filename', None) or ''
                filename = filename.strip() if filename else ''
                
                # Only process if filename is not empty
                if filename:
                    from .utils import validate_logo_upload
                    ...
                    if is_valid:
                        ...
                        # Only show error if validation failed (not for empty files)
                        if error_msg:  # Only flash if there's an actual error message
                            flash(f'خطا در آپلود لوگو: {error_msg}', 'error')
                            logger.warning(f"Logo upload validation failed: {error_msg}")
                # If filename is empty, do nothing (no error - file upload is optional)
            ...
            if survey.access_type == 'anonymous':
                ...
                # Validate maximum number of parameters (5)
                MAX_PARAMETERS = 5
                valid_parameters_count = 0
                for idx in range(len(parameter_names)):
                    if idx < len(parameter_names) and parameter_names[idx] and parameter_names[idx].strip():
                        if idx < len(parameter_values_list) and parameter_values_list[idx] and parameter_values_list[idx].strip():
                            valid_parameters_count += 1
                
                if valid_parameters_count > MAX_PARAMETERS:
                    flash(f'حداکثر {MAX_PARAMETERS} پارامتر می‌توانید تعریف کنید. شما {valid_parameters_count} پارامتر ارسال کرده‌اید.', 'error')
                    logger.warning(f"Too many parameters submitted: {valid_parameters_count} (max: {MAX_PARAMETERS})")
                    # Don't process parameters if limit exceeded
                    parameter_names = []
                ...
                for pid in parameter_ids:
                    if pid and pid.strip():
                        try:
                            submitted_param_ids.append(int(pid))
                ...
                # استفاده از max برای اطمینان از اینکه همه لیست‌ها پردازش شوند
                max_length = max(len(parameter_names), len(parameter_values_list), len(parameter_ids))
                
                for idx in range(max_length):
                    # دریافت مقادیر با بررسی index
                    param_name = parameter_names[idx].strip() if idx < len(parameter_names) and parameter_names[idx] else None
                    param_values_str = parameter_values_list[idx].strip() if idx < len(parameter_values_list) and parameter_values_list[idx] else None
                    
                    # اگر هر دو خالی باشند، skip کنید
                    ...
                    # دریافت parameter_id
                    param_id = parameter_ids[idx].strip() if idx < len(parameter_ids) and parameter_ids[idx] else None
                    ...
                    if param_id and param_id.strip():
                        ...
                        try:
                            ...
                            if param and param.survey_id == survey.id:
                                ...
                                # اگر parameter پیدا نشد یا متعلق به survey دیگری است، به عنوان جدید ایجاد کنید
                            ...
                            logger.warning(f"Error updating parameter {param_id}: {e}")
                            # اگر update نشد، به عنوان جدید ایجاد کنید
                            param = SurveyParameter(
                                survey_id=survey.id,
                                parameter_name=param_name,
                                parameter_values=param_values,
    ...
    if request.method == 'POST':
        try:
            ...
            if start_date_str:
                try:
                    # Parse Jalali date (format: YYYY/MM/DD)
                    jalali_parts = list(map(int, start_date_str.split('/')))
                    if len(jalali_parts) == 3:
                        jalali_dt = jdatetime(jalali_parts[0], jalali_parts[1], jalali_parts[2])
                        survey.start_date = jalali_dt.togregorian()
                        logger.info(f"Updated survey {survey_id} start_date to {survey.start_date}")
                    else:
                        logger.warning(f"Invalid start_date format: {start_date_str} - expected YYYY/MM/DD")
                        flash(f'فرمت تاریخ شروع نامعتبر است. فرمت صحیح: YYYY/MM/DD', 'warning')
                except (ValueError, TypeError, AttributeError) as e:
                    logger.error(f"Error parsing start_date '{start_date_str}': {e}", exc_info=True)
                    flash(f'خطا در parse کردن تاریخ شروع: {str(e)}', 'error')
                    # Don't change the date if parsing fails
                except Exception as e:
                    logger.error(f"Unexpected error parsing start_date '{start_date_str}': {e}", exc_info=True)
                    flash(f'خطای غیرمنتظره در parse کردن تاریخ شروع', 'error')
            else:
                # If field is empty, set to None (to clear the date)
                survey.start_date = None
                logger.info(f"Cleared start_date for survey {survey_id}")
            
            end_date_str = request.form.get('end_date', '').strip()
            if end_date_str:
                try:
                    # Parse Jalali date (format: YYYY/MM/DD)
                    jalali_parts = list(map(int, end_date_str.split('/')))
                    if len(jalali_parts) == 3:
                        jalali_dt = jdatetime(jalali_parts[0], jalali_parts[1], jalali_parts[2])
                        survey.end_date = jalali_dt.togregorian()
                        logger.info(f"Updated survey {survey_id} end_date to {survey.end_date}")
                    else:
                        logger.warning(f"Invalid end_date format: {end_date_str} - expected YYYY/MM/DD")
                        flash(f'فرمت تاریخ پایان نامعتبر است. فرمت صحیح: YYYY/MM/DD', 'warning')
                except (ValueError, TypeError, AttributeError) as e:
                    logger.error(f"Error parsing end_date '{end_date_str}': {e}", exc_info=True)
                    flash(f'خطا در parse کردن تاریخ پایان: {str(e)}', 'error')
                    # Don't change the date if parsing fails
                except Exception as e:
                    logger.error(f"Unexpected error parsing end_date '{end_date_str}': {e}", exc_info=True)
                    flash(f'خطای غیرمنتظره در parse کردن تاریخ پایان', 'error')
            else:
                # If field is empty, set to None (to clear the date)
                survey.end_date = None
                logger.info(f"Cleared end_date for survey {survey_id}")
            ...
            if survey.access_type == 'anonymous':
                ...
                logger.info(f"Processing parameters for survey {survey_id}: names={len(parameter_names)}, values={len(parameter_values_list)}, ids={len(parameter_ids)}")
                logger.info(f"Backend received - Parameter names: {parameter_names}")
                logger.info(f"Backend received - Parameter values: {parameter_values_list}")
                logger.info(f"Backend received - Parameter IDs: {parameter_ids}")
                
                # Also log raw form data for debugging
                logger.info(f"Backend - All form keys: {list(request.form.keys())}")
                logger.info(f"Backend - request.form.getlist('parameter_name[]'): {request.form.getlist('parameter_name[]')}")
                logger.info(f"Backend - request.form.getlist('parameter_values[]'): {request.form.getlist('parameter_values[]')}")
                logger.info(f"Backend - request.form.getlist('parameter_id[]'): {request.form.getlist('parameter_id[]')}")
                
                # Delete parameters that are not in the form
                existing_params = {p.id: p for p in SurveyParameter.query.filter_by(survey_id=survey.id).all()}
                submitted_param_ids = []
                for pid in parameter_ids:
                    # حتی اگر "999999" یا "0" باشد، باید پردازش شود (برای پارامترهای جدید)
                    if pid and pid.strip() and pid.strip() not in ["0", "999999", ""]:
                        try:
                            submitted_param_ids.append(int(pid))
                        except (ValueError, TypeError):
                            pass
                
                logger.info(f"Existing params: {list(existing_params.keys())}, Submitted IDs: {submitted_param_ids}")
                
                for param_id, param in existing_params.items():
                    if param_id not in submitted_param_ids:
                        logger.info(f"Deleting parameter {param_id} (not in submitted list)")
                        db.session.delete(param)
                
                # Add or update parameters
                # استفاده از max برای اطمینان از اینکه همه لیست‌ها پردازش شوند
                # اگر همه لیست‌ها خالی باشند، max_length = 0 می‌شود که درست است
                # CRITICAL: استفاده از max از parameter_names و parameter_values_list (نه parameter_ids)
                # چون parameter_ids ممکن است خالی باشد و Flask آن را نادیده بگیرد
                max_length = max(len(parameter_names), len(parameter_values_list)) if (parameter_names or parameter_values_list) else 0
                
                logger.info(f"Max length for processing: {max_length} (names={len(parameter_names)}, values={len(parameter_values_list)}, ids={len(parameter_ids)})")
                ...
                for idx in range(max_length):
                    ...
                    if idx < len(parameter_names) and parameter_names[idx]:
                        param_name = str(parameter_names[idx]).strip()
                    
                    if idx < len(parameter_values_list) and parameter_values_list[idx]:
                        param_values_str = str(parameter_values_list[idx]).strip()
                    
                    # اگر هر دو خالی باشند، skip کنید
                    if not param_name or not param_values_str:
                        logger.debug(f"Skipping index {idx}: name={param_name}, values={param_values_str}")
                        continue
                    ...
                    if not param_values:
                        logger.warning(f"Parameter {param_name} has no valid values, skipping")
                        continue
                    
                    # دریافت parameter_id
                    param_id = None
                    if idx < len(parameter_ids):
                        # حتی اگر خالی باشد، باید پردازش شود
                        param_id_str = str(parameter_ids[idx]).strip() if parameter_ids[idx] else ""
                        # اگر مقدار "999999" یا "0" یا خالی باشد، به معنای پارامتر جدید است
                        if param_id_str and param_id_str not in ["0", "999999", ""]:
                            try:
                                # بررسی کنید که آیا می‌تواند به int تبدیل شود و مثبت است و کمتر از 999999
                                param_id_int = int(param_id_str)
                                if param_id_int > 0 and param_id_int < 999999:
                                    param_id = param_id_str
                            except ValueError:
                                # اگر نمی‌تواند به int تبدیل شود، به معنای پارامتر جدید است
                                param_id = None
                    else:
                        # اگر parameter_ids کوتاه‌تر از parameter_names باشد، param_id باید None باشد
                        param_id = None
                    
                    logger.info(f"Processing parameter at index {idx}: name={param_name}, values={param_values}, id={param_id} (raw: {parameter_ids[idx] if idx < len(parameter_ids) else 'N/A'})")
                    
                    if param_id and param_id.strip() and param_id not in ["0", "999999"]:
                        # Update existing parameter
                        try:
                            param = SurveyParameter.query.get(int(param_id))
                            if param and param.survey_id == survey.id:
                                logger.info(f"Updating existing parameter {param_id}: {param_name}")
                                
                                # CRITICAL: Check if the new name conflicts with another parameter
                                # (excluding the current parameter being updated)
                                if param.parameter_name != param_name:
                                    # Name is being changed - check for conflicts
                                    conflicting_param = SurveyParameter.query.filter_by(
                                        survey_id=survey.id,
                                        parameter_name=param_name
                                    ).filter(SurveyParameter.id != int(param_id)).first()
                                    
                                    if conflicting_param:
                                        # Conflict detected - delete the conflicting parameter first
                                        logger.warning(f"Parameter name conflict: '{param_name}' already exists with id={conflicting_param.id}. Deleting conflicting parameter.")
                                        db.session.delete(conflicting_param)
                                ...
                                # اگر parameter پیدا نشد یا متعلق به survey دیگری است، به عنوان جدید ایجاد کنید
                                logger.warning(f"Parameter {param_id} not found or belongs to different survey, creating new")
                                
                                # Check if parameter with this name already exists
                                existing_param = SurveyParameter.query.filter_by(
                                    survey_id=survey.id,
                                    parameter_name=param_name
                                ).first()
                                
                                if existing_param:
                                    # Update existing parameter instead of creating duplicate
                                    logger.info(f"Parameter with name '{param_name}' already exists (id={existing_param.id}), updating it instead")
                                    existing_param.parameter_values = param_values
                                    existing_param.order = idx + 1
                                else:
                                    param = SurveyParameter(
                                        survey_id=survey.id,
                                        parameter_name=param_name,
                            ...
                            logger.warning(f"Error updating parameter {param_id}: {e}, creating new")
                        ...
                        # Create new parameter
                        logger.info(f"Creating new parameter: {param_name}")
                        param = SurveyParameter(
                            survey_id=survey.id,
                            parameter_name=param_name,
                            parameter_values=param_values,
...
def manager_reports_export_links_excel(survey_id):
    ...
    try:
        ...
        if parameters:
            ...
            # No parameters - just create a single link
            links_data.append({'ردیف': 1, 'url': base_url})
            column_order = ['ردیف', 'url']
...
def manager_reports_export_links_qr_zip(survey_id):
    ...
    try:
        ...
        # Generate URLs list with parameter info
        # Format: (idx, full_url, param_names, combo) or (idx, full_url, None, None) for no params
        urls_to_process = []
        ...
        if parameters:
            ...
            for idx, combo in enumerate(combinations, start=1):
                ...
                url_params = '&'.join([f"{name}={quote(str(value), safe='')}" for name, value in params_dict.items()])
                full_url = f"{base_url}&{url_params}"
                urls_to_process.append((idx, full_url, param_names, combo))
        else:
            # No parameters - just create a single link
            urls_to_process.append((1, base_url, None, None))
        ...
        with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
            for idx, full_url, param_names, combo in urls_to_process:
